...
#Ensure motor speed is set to zero immediatly
...
#Wait for instruciton to start up flight sequence
feedback = None
while feedback != "START":
    feedback = input("Startup Sequence Passcode: ")
...
for i in range(20, 190):
    ...
    time.sleep(0.2)
print("Initial Motor Speed Reached")
print("-------------------------------------------------------------------------------------------------------------")

# Run feedback loop which will take user input 
count = 0
while feedback != "X":
    feedback = input("Enter Command (Move, Collect Data, Terminate) M/D/X: ")
    if feedback == 'X':
        break
    elif feedback == 'D':
        # Capture Image and Run through model
        file = 'Image{count}.jpg'
        if camera.capture_image(file):
        ...
        speed = int(input("Wanted Speed Change: "))
        ...
        print("PWM Signal Now At: ", motor_FL.speed)
